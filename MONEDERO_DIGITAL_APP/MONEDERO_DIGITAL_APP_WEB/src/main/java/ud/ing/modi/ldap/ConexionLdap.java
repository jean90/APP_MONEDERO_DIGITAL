...
public class ConexionLdap {
    ...
    /**
     * Este metodo permite realizar la conexion al servidor de LDAP Para el
     * usuario manager
     *
     * @param strUser
     * @param strPassword
    ...
    public LDAPConnection ConexionUser(String strUser, String strPassword) {
        String login = "uid=" + strUser + ",ou=Users,dc=monederodigital,dc=com,dc=co";
        System.out.println("" + login);
    ...
    public void abrirConexionLdap() {
        ...
        try {
            ...
            Logger.getLogger(ConexionLdap.class.getName()).log(Level.SEVERE, "Error 1 en abrirConexionLdap " + ex);
        } catch (LDAPException ex) {
            Logger.getLogger(ConexionLdap.class.getName()).log(Level.SEVERE, "Error 2 en abrirConexionLdap " + ex);
        }

    }

    /**
     * Este método abre una conexón ldap no con el usuario de administración del ldap, sino con otro en específico.
     * @param usuario Es el usuario con el que se abrirá la conexión.
     * @param password  Es la contraseña de acceso del usuario.
     */
    public void abrirConexionLdap(String usuario, String password) {
        
        String login = "uid=" + usuario + ",ou=Users,dc=monederodigital,dc=com,dc=co";
        try {
            this.lc = new LDAPConnection();
            if (this.lc.isConnectionAlive()) {
                System.out.println("conexion abierta");
            ...
            Logger.getLogger(ConexionLdap.class.getName()).log(Level.SEVERE,  "Error 1 en abrirConexionLdap con usu " + ex);
        } 
    }
